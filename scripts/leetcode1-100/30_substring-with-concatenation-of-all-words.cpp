    ...
    vector<int> findSubstring(string s, vector<string> &words) {
        ...
        for (int i = 0; i < words.size(); ++i) {
            if (mp1.find(words[i]) == mp1.end()) {
                mp1.insert(std::pair<string, int>(words[i], 1));
                mp2.insert(std::pair<string, int>(words[i], 0));
            } else {
                mp1[words[i]] += 1;
            }
        }
        ...
        for (int j = 0; j < word_len; ++j) {
            ...
            for (int i = 0; i <= tmp_s.size() / word_len - words.size(); ++i) {
                if (i == 0) {
